...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  const statusConfig = {
    ongoing: { label: tr('status.ongoing', 'EN COURS'), color: theme.colors.primary },
    completed: { label: tr('status.completed', 'TERMINÉ'), color: theme.colors.success },
    dropped: { label: tr('status.dropped', 'ABANDONNÉ'), color: theme.colors.textSecondary },
  };
  const config = statusConfig[status as keyof typeof statusConfig] ?? statusConfig.ongoing;
  return <View style={[styles.badge, { borderColor: config.color }]}><Text style={[styles.badgeText, { color: config.color }]}>{config.label}</Text></View>;
};

const MangaCard = ({ manga, onDelete, onPress }: { manga: Manga; onDelete: (id: string) => void; onPress: (manga: Manga) => void }) => {
  const { theme } = useTheme();
  const { tr } = useLanguage();
  const styles = makeStyles(theme);
  return (
    <TouchableOpacity style={styles.card} onPress={() => onPress(manga)}>
      <View style={styles.cardCover}><Text style={styles.cardEmoji}>📖</Text></View>
      <View style={styles.cardInfo}>
        <Text style={styles.cardTitle}>{manga.title.toUpperCase()}</Text>
        <Text style={styles.cardChapter}>{tr('manga.chapter_label', 'Chapitre')} {manga.current_chapter}</Text>
        <StatusBadge status={manga.status} />
        {manga.rating && <Text style={styles.cardRating}>{'⭐'.repeat(manga.rating)}</Text>}
      </View>
      <TouchableOpacity style={styles.deleteButton} onPress={() => onDelete(manga.id)}><Text style={styles.deleteText}>🗑️</Text></TouchableOpacity>
    </TouchableOpacity>
  );
};

export const MangaListScreen = ({ onSelectManga, onAddManga, onBack, onSettings, listId }: { listId: string; onSelectManga: (manga: Manga) => void; onAddManga: () => void; onBack: () => void; onSettings: () => void }) => {
  ...
  const fetchMangas = async () => {
    try { const data = await getMangas(listId); setMangas(data); setFiltered(data); }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{tr('manga.collection', '🌸 Ma Collection')}</Text>
        <TouchableOpacity onPress={onSettings}><Text style={{ fontSize: theme.fontSize.xl }}>⚙️</Text></TouchableOpacity>
      </View>
      <View style={styles.searchContainer}>
        <TextInput style={styles.searchInput} placeholder={tr('manga.search', '🔍 Rechercher un manga...')} value={search} onChangeText={setSearch} placeholderTextColor={theme.colors.textSecondary} />
        {search.length > 0 && <TouchableOpacity onPress={() => setSearch('')}><Text style={styles.clearSearch}>✕</Text></TouchableOpacity>}
      </View>
      {filtered.length === 0 ? (
        <View style={styles.empty}>
          <Text style={styles.emptyEmoji}>🌸</Text>
          <Text style={styles.emptyTitle}>{search.length > 0 ? tr('lists.no_results', 'Aucun résultat') : tr('manga.empty', 'Aucun manga pour l\'instant')}</Text>
          <Text style={styles.emptySubtitle}>{search.length > 0 ? `${tr('lists.no_results_for', 'Pas de manga pour')} "${search}"` : tr('manga.empty.subtitle', 'Ajoute ton premier manga !')}</Text>
        </View>
      ) : (
        <FlatList data={filtered} keyExtractor={(item) => item.id} renderItem={({ item }) => <MangaCard manga={item} onDelete={handleDelete} onPress={onSelectManga} />} contentContainerStyle={styles.list} />
      )}
      <TouchableOpacity style={styles.fab} onPress={onAddManga}><Text style={styles.fabText}>+</Text></TouchableOpacity>
    </SafeAreaView>
  );
};
...
const makeStyles = (theme: Theme) => StyleSheet.create({
  ...
  cardCover: { width: 56, height: 80, backgroundColor: theme.colors.accent, borderRadius: theme.borderRadius.sm, justifyContent: 'center', alignItems: 'center' },
  ...
  badge: { alignSelf: 'flex-start', borderWidth: 1, borderRadius: theme.borderRadius.full, paddingHorizontal: theme.spacing.sm, paddingVertical: 2, marginTop: 4 },
  ...
  headerCount: { fontSize: theme.fontSize.md, color: theme.colors.textSecondary },
});
