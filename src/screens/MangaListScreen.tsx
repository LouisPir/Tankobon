...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  const statusConfig = {
    ongoing: { label: t('status.ongoing'), color: theme.colors.primary },
    completed: { label: t('status.completed'), color: theme.colors.success },
    dropped: { label: t('status.dropped'), color: theme.colors.textSecondary },
  };
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={() => onPress(manga)}>
      ...
      <View style={styles.cardInfo}>
        ...
        <Text style={styles.cardChapter}>{t('manga.chapter_label')} {manga.current_chapter}</Text>
        <StatusBadge status={manga.status} />
        {manga.rating && <Text style={styles.cardRating}>{'⭐'.repeat(manga.rating)}</Text>}
      </View>
      <TouchableOpacity style={styles.deleteButton} onPress={() => onDelete(manga.id)}>
...
export const MangaListScreen = ({
  ...
  useEffect(() => {
    if (search.trim() === '') {
      ...
      setFiltered(mangas.filter((m) => m.title.toLowerCase().includes(search.toLowerCase())));
    }
  }, [search, mangas]);

  const handleDelete = (id: string) => {
    Alert.alert(t('delete'), t('manga.delete.confirm'), [
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t('manga.collection')}</Text>
      ...
      {filtered.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyTitle}>
            {search.length > 0 ? t('lists.no_results') : t('manga.empty')}
          </Text>
          <Text style={styles.emptySubtitle}>
            {search.length > 0 ? `${t('lists.no_results_for')} "${search}"` : t('manga.empty.subtitle')}
          </Text>
        </View>
      ) : (
        <FlatList
          ...
          renderItem={({ item }) => (
            <MangaCard manga={item} onDelete={handleDelete} onPress={onSelectManga} />
          )}
