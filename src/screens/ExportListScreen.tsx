  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{tr('export.title', 'Exporter une liste')}</Text>
        <View style={{ width: 60 }} />
      </View>
      <View style={styles.content}>
        {preselectedList ? (
          <View style={styles.preselectedCard}>
            <Text style={styles.preselectedLabel}>{tr('export.selected', 'Liste sélectionnée')}</Text>
            <Text style={styles.preselectedName}>{preselectedList.name}</Text>
          </View>
        ) : (
          <>
            <Text style={styles.label}>{tr('export.select', 'Choisir une liste *')}</Text>
            ...
            {listsLoading ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity style={[styles.listOption, selectedList?.id === item.id && styles.listOptionSelected]} onPress={() => setSelectedList(item)}>
                    <Text style={[styles.listOptionText, selectedList?.id === item.id && styles.listOptionTextSelected]}>{item.name}</Text>
        ...
        <TouchableOpacity style={[styles.exportButton, (!selectedList || loading) && styles.exportButtonDisabled]} onPress={handleExport} disabled={!selectedList || loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.exportButtonText}>{tr('export.button', 'Exporter 📤')}</Text>}
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
...
const makeStyles = (theme: Theme) => StyleSheet.create({
  ...
  exportButton: { backgroundColor: theme.colors.primary, padding: theme.spacing.md, borderRadius: theme.borderRadius.full, alignItems: 'center', marginTop: 'auto' },
